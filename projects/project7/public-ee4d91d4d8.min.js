...
(function ($) {
	...
	zl.problem = {
		...
		list : function (transData) {
			...
			//swiper参数设置
			var max_len = sliders_arr.length <= 70 ? 16 : 60;
			...
			zl.swiperProblem = new Swiper('.swiper-problem', {
				...
				paginationCustomRender: function (swiper, current, total) {
					if (typeof $(swiper.slides[current-1])[0] != 'undefined') {
			...
			if (zl.exam_type == '1') {
				var callback = function(object) {
					...
					if (val == zl.page_num[order][1]) {
						$('#answerAnther' + order).addClass('answer_success').text('回答正确！').siblings('span').hide();
						//更新答题卡数据和题号样式
						zl.swiper_problem_parm.ques_ok_num++;
						//$('#answer-cont')
						if (type == '2') {
							if (pageNumber - 1 == mySwiper[swiperChild].activeIndex) {
								zl.swiperProblem.slideNext();
		...
		saveActiveSlide: function () {
			//保存当前slide
			var swiper = zl.swiperProblem;
			var active_slide = swiper.slides[swiper.activeIndex];
			var active_slide_index = $(active_slide).data('index');
			(zl.swiper_problem_parm.parent_sliders)[active_slide_index] = active_slide;
		},
		submitTip : function () {
			//提交
			zl.problem.saveActiveSlide();//保存当前slide
			var ok_answer = 0;
		...
		submitData : function () {
			...
			if (zl.exam_type == '1') {
				...
				//$('#answer-number').text(zl.swiper_problem_parm.ques_ok_num);
			}
			...
			if (transPageIndex != 'hide') {
				...
				if (parent_arr.length > max_num) {
					var n = transPageIndex,
						...
						total = zl.swiper_problem_parm.data_len-1,
						temp_arr = [n-left, n+(max-left-1)],
						res = [temp_arr[0], temp_arr[1]];
					if (temp_arr[0] < 0) {
						res[0] = 0;
						res[1] = temp_arr[1]-temp_arr[0];
					} else if (temp_arr[1] > total) {
						res[0] = temp_arr[0]-(temp_arr[1]-total);
						res[1] = total;
					}
					var append_arr = parent_arr.slice(res[0], res[1]+1);
					zl.swiperProblem.removeAllSlides();
					zl.swiperProblem.appendSlide(append_arr);
