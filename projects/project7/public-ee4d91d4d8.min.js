	...
	zl.problem = {
		...
		pageIndexSwitch : function (transPageIndex) {
			...
			if (transPageIndex != 'hide') {
				var temp_arr = zl.swiper_problem_parm.parent_sliders;
				var lastIndex = transPageIndex,
					max_num = zl.swiper_problem_parm.max_sliders_num;
				if (temp_arr.length > max_num) {
					var page = transPageIndex+1;
					var mark = zl.swiper_problem_parm.front_point-1;
					var mark_last;
					...
					var get_arr = [page-mark_last-1, page+zl.swiper_problem_parm.max_sliders_num-(mark_last+1)-1];
					zl.swiperProblem.appendSlide(temp_arr.slice(get_arr[0], get_arr[1]+1));
					var num_arr = [];
					for (var i = 0, iL = zl.swiper_problem_parm.max_sliders_num; i < iL; i++) {
						num_arr.push(i);
					}
					zl.swiperProblem.removeSlide(num_arr);
					lastIndex = transPageIndex - get_arr[0];
				}
