...
(function ($) {
	var zl = {
		...
		ques_num : 0,
		page_num : {},
		max_sliders_num: 16,//同时最多显示16个slider
		exam_type : '1',
	...
	zl.problem = {
		...
		list : function (transData) {
			...
			var max_len = zl.max_sliders_num,
				front_point = max_len*(1/4),
				back_point = max_len*(3/4),// 4 12
				btw = front_point,
			  data_len = sliders_arr.length;
			zl.page_num.need_refresh = (data_len > max_len) ? true : false;
			zl.page_num.parent_sliders = sliders_arr;//保存整个父级sliders数组
			//初始化swiper
			var init_child_swiper = [],//已经初始化的子swiper
				init_slide_index = 0,
				move_slide_index;

			$('.swiper-problem').html(str_start + sliders_arr.slice(init_slide_index, max_len+init_slide_index).join('') + str_end);
			zl.swiperProblem = new Swiper('.swiper-problem', {
				nextButton : '.swiper-button-next',
				prevButton : '.swiper-button-prev',
				pagination : '.swiper-pagination',
				...
				paginationCustomRender: function (swiper, current, total) {
					return (parseInt($(swiper.slides[current-1]).data('index'))+1) + ' / ' + data_len;
				},
				speed: 0,
				onInit: function (swiper) {
					...
					if (init_child_swiper.indexOf(child_swiper_id) == -1 && $(child_swiper_id).length > 0) {
						var mySwiper = new Swiper(child_swiper_id, {
							nested : true
						});
				...
				onSlideChangeEnd: function(swiper) {
					...
					//少于最大slides数，不需要更新
					if (!zl.page_num.need_refresh) return;
		...
		pageIndexSwitch : function (transPageIndex) {
			...
			if (transPageIndex != 'hide') {
				var temp_arr = $.zl.page_num.parent_sliders;
				var lastIndex = transPageIndex;
				if (temp_arr.length > $.zl.max_sliders_num) {
					$.zl.swiperProblem.appendSlide(temp_arr.slice(3,19));
					var num_arr = [];
					for (var i=0;i<$.zl.max_sliders_num;i++) {
						num_arr.push(i);
					}
					$.zl.swiperProblem.removeSlide(num_arr);
				} else {
					$.zl.swiperProblem.slideTo(transPageIndex);
				}
			}
		},
